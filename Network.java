...
public boolean addFollowee(String name1, String name2) {
 ...
 /*  public String recommendWhoToFollow(String name) {
    ...
    public String recommendWhoToFollow(String name) {
        ...
        for (int i = 0; i < users.length; i++) {
            if (users[i] != null && !users[i].getName().equalsIgnoreCase(name)) {
                int mutualCount = users[i].countMutual(user1);
                if (mutualCount > max) {
                    maxmutual = users[i];
                    max = mutualCount;
                }
        ...
        return maxmutual == null ? null : maxmutual.getName();
    }
    ...
    public String mostPopularUser() {
        User maxFollowers = null;
        int max = 0;
        for (int i = 0; i < userCount; i ++) {
            if (users[i] != null) {
            int followeeCount = followeeCount(users[i].getName());
           if (max < followeeCount) {
            maxFollowers = users[i];
            max = followeeCount;
           }
        }
      }
      if (maxFollowers != null) {
        return maxFollowers.getName();
        } else {
        return null;
        }
    }
